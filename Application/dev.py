...
def show_target_details_page():
    ...
    if trigger or st.session_state.trigger_state:
        ...
        st.write("List of skills triggered:")
        skills = trigger_skills(description)
        if len(skills) > 0:
            st.success(skills)
    ...
    # Display triggered skills if available
    if "triggered_skills" in st.session_state:
        st.write("List of skills triggered:")
        st.success(st.session_state.triggered_skills)
...
def sortscores(data):
    ...
    if st.button("NOTIFY INTERVIEW"):
        notify_interview(selected_candidates, interview_date, interview_time, interview_location)
...
def notify_interview(selected_candidates, interview_date, interview_time, interview_location):
    for candidate in selected_candidates:
